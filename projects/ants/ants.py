...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(0, self.min_range):
            find_bees = find_bees.entrance
            j += 1
        while not find_bees.bees and find_bees != hive and j < self.max_range:
            find_bees = find_bees.entrance
            j += 1
        if find_bees == hive:
            return None
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        while x is not None:
            if x.ant:
                if x.ant not in self.new_ants:
                    x.ant.damage *= 2
                    self.new_ants.append(x.ant)
                if isinstance(x.ant, BodyguardAnt) or isinstance(x.ant, TankAnt):
                    if x.ant.contained_ant is not None and isinstance(x.ant.contained_ant, FireAnt) == False:
                        x.ant.contained_ant.damage *= 2
                        self.new_ants.append(x.ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        ...
        if counter < duration:
            counter += 1
            return effect(old_action, bee)(colony)
        else:
